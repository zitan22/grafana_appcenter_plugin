...
export class DataSource extends DataSourceApi<MyQuery, MyDataSourceOptions> {
  ...
  async query(options: DataQueryRequest<MyQuery>): Promise<DataQueryResponse> {
    ...
    const promises = options.targets.map((query) => {
      ...
      switch (query.type.value) {
        ...
        case 'Errors details':
          return this.listErrorsDetails(query);
      }
  ...
  async listErrorsDetails(query: MyQuery) {
    const errorGroupId = this.getVariable('errorGroupId');
    let url = `${this.baseUrl}` + `/v0.1/apps/{owner_name}/{app_name}/errors/errorGroups/${errorGroupId}/errors/latest`;
    const params = {
      start: this.start.toISOString(),
      end: this.end.toISOString(),
    };
    ...
    this.appName.split(';').forEach(async (appName) => {
      url = url.replace('{owner_name}', this.orgName).replace('{app_name}', appName);
    });

    return await this.doRequest(url, params).then((response) => {
      ...
      const frame = new MutableDataFrame({
        ...
        fields: [
          ...
          { name: 'Device', type: FieldType.string },
          { name: 'OS Version', type: FieldType.string },
          { name: 'OS Type', type: FieldType.string },
          { name: 'Sn', type: FieldType.string },
          { name: 'App Version', type: FieldType.string },
          { name: 'Details', type: FieldType.string },
          { name: 'Date', type: FieldType.string },
        ],
      });
